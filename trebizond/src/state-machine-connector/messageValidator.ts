...
import { SignedObject, checkObjectSignature } from '../trebizond-common/crypto';
import { Operation, OpMessage } from '../trebizond-common/datatypes';

export abstract class MessageValidator<Op extends Operation> {

    public validate(message: SignedObject<OpMessage<Op>>, publicKey: Buffer): boolean {
        return this.authenticationValidation(message, publicKey)
            && this.semanticValidation(message.value.operation.operation);
    }

    public authenticationValidation(message: SignedObject<OpMessage<Op>>, publicKey: Buffer): boolean {
